...
parser.add_argument('--dataset', default='STARE', choices=['STARE'])
parser.add_argument('--data_path', type=str, default='datasets/', help='data path')
parser.add_argument('--fold', type=int, default=0, help='20-Fold (default: 0 - 19)')
...
parser.add_argument('--patch_num', type=int, default=38000, help='patchs number (default: 800000)')
parser.add_argument('--patch_size', type=int, default=48, help='patch size (default: 48)')

parser.add_argument('--stride_size', type=int, default=5, help='stride_size (default: 5)')
...
parser.add_argument('--occ_func', default='fill_next_tar',
...
# 加载数据集
train_orig_imgs, train_orig_gts, train_orig_masks, test_orig_imgs, test_orig_gts, test_orig_masks = get_orig_SATRE_datasets(
    args)
...
def train():
    ...
    for epoch in range(cur_epoch, args['epochs']):
        ...
        pred_imgs = recompone_overlap(pred_patches, new_height, new_width, args['stride_size'])
        pred_imgs = pred_imgs[:, :, 0:test_orig_imgs.shape[2], 0:test_orig_imgs.shape[3]]
        # test_orig_gts = test_orig_gts[:, :, 0:test_orig_imgs.shape[2], 0:test_orig_imgs.shape[3]]
        kill_border(pred_imgs, test_orig_masks)
        '''if vis is not None:
            vis_mask = vis.image(make_grid(torch.from_numpy(test_orig_gts), nrow=1, normalize=True, scale_each=True),
                                 opts=dict(title='Test Masks'),
                                 win=vis_mask)
            vis_outputs = vis.image(make_grid(torch.from_numpy(pred_imgs), nrow=1, normalize=True, scale_each=True),
                                    opts=dict(title='Test Pred'),
                                    win=vis_outputs)'''

        y_scores, y_true = pred_only_FOV(pred_imgs, test_orig_gts, test_orig_masks)
