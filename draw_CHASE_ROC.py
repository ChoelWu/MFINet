...
data_path = './datasets/CHASE/'  # 数据集路径
...
model_path_UNet = '/home/sdc_3_7T/jiangyun/wuchao/MSINet/logs/CHASE/UNet/2/checkpoints/periods/17_result_average/'
pred_imgs_UNet = np.load(model_path_UNet + 'pred_imgs.npy')

model_path_UNetMI = '/home/sdc_3_7T/jiangyun/wuchao/MSINet/logs/CHASE/UNetMI/1/checkpoints/last_result_average/'
pred_imgs_UNetMI = np.load(model_path_UNetMI + 'pred_imgs.npy')

model_path_UNetFG = '/home/sdc_3_7T/jiangyun/wuchao/MSINet/logs/CHASE/UNetFG/2/checkpoints/periods/70_result_average/'
pred_imgs_UNetFG = np.load(model_path_UNetFG + 'pred_imgs.npy')

model_path_UNetMR = '/home/sdc_3_7T/jiangyun/wuchao/MSINet/logs/CHASE/UNetMR/2/checkpoints/periods/107_result_average/'
pred_imgs_UNetMR = np.load(model_path_UNetMR + 'pred_imgs.npy')

model_path_UNetMRFG = '/home/sdc_3_7T/jiangyun/wuchao/MSINet/logs/CHASE/UNetMRFG/1/checkpoints/periods/158_result_average/'
...
plt.plot(fpr_UNetMI, tpr_UNetMI, label='UNet+MI (AUC = %0.4f)' % AUC_ROC_UNetMI, color='yellow')
...
plt.axis([0, 1, 0.94, 1])
plt.title('CHASE ROC curve', font_title)
plt.savefig("./pred/CHASE_AUC_ROC.eps")
plt.show()
...
plt.plot(recall_UNet, precision_UNet, '-', label='UNet (AUC = %0.4f)' % 0.8095859101527202, color='black')
plt.plot(recall_UNetMI, precision_UNetMI, '-', label='UNet+MI (AUC = %0.4f)' % 0.8304042641849386, color='yellow')
plt.plot(recall_UNetFG, precision_UNetFG, '-', label='UNet+FG (AUC = %0.4f)' % 0.8359254251104107, color='red')
plt.plot(recall_UNetMR, precision_UNetMR, '-', label='UNet+MR (AUC = %0.4f)' % 0.8344408166540485, color='green')
plt.plot(recall_UNetMRFG, precision_UNetMRFG, '-', label='UNet+FG+MR (AUC = %0.4f)' % 0.8375560383015296, color='blue')

plt.title('CHASE Precision Recall curve', font_title)
plt.xlabel("Recall", font)
plt.ylabel("Precision", font)
plt.legend(loc="lower left", prop=font_legend)
plt.axis([0, 1, 0.60, 1])
plt.savefig("./pred/CHASE_AUC_PR.eps")
plt.show()
